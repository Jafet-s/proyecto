...
<body>
    ...
    <div class="container mt-5">
        ...
        <form action="{{ url('/actualizar-apiAdm/' . $data['id_administrador']) }}" method="POST">
            @csrf
            @method('PUT') <!-- Esto convierte el formulario en una solicitud PUT -->
            
            <div class="form-group">
                <label for="nombre"><i class="bi bi-card-text"></i> Nombre</label>
                <input type="text" class="form-control" name="nombre" value="{{ $data['nombre'] }}">
            </div>
            
            <div class="form-group">
                <label for="telefono"><i class="bi bi-person"></i> Telefono</label>
            ...
            <div class="form-group">
                <label for="correo"><i class="bi bi-person"></i> Correo</label>
                <input type="text" class="form-control" name="correo" value="{{ $data['correo'] }}">
            </div>

            <div class="form-group">
                <label for="contraseña"><i class="bi bi-calendar"></i> Contraseña</label>
                <input type="text" class="form-control" name="contraseña" value="{{ $data['contraseña'] }}">
            </div>

            <button type="submit" class="btn btn-primary">
                <i class="bi bi-save"></i> Actualizar
            </button>
            <a href="{{ url('/consultar-apiAdm') }}" class="btn btn-secondary">
